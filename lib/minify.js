...
function main(pom) {
    'use strict';
    var images = {}, image, rule;
    pom.styleSheet.cssRules.forEach(function (cssRule) {
        // 如果是CSS function 那么没有 cssRule.style
        if (cssRule.style) {
            image = cssRule.style.getPropertyValue('background-image');
            if (image) {
                images[image] = images[image] || {
                    role: []
                };
                images[image].role.push(cssRule.selectorText);
                cssRule.style.removeProperty('background-image');
            }
        }
    });
    for (var o in images) {

        if (images.hasOwnProperty(o)) {
            rule = images[o].role.join(',') + "{background-image:" + o + ";}";
            //插入到第二个位置，避免和@charset 争第一行。
            pom.styleSheet.insertRule(rule, 1);
        }
    }

    //return YAHOO.compressor.cssmin(pom.styleSheet.toString(), 0);
    return pom.styleSheet.toString();
}
