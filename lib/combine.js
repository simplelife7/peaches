...
Combine.prototype = {
    _init:function () {
        ...
        async.forEach(this.packers, function (packer, callback) {
            ...
            try {
                ...
                logger.error('无法加载：%s,使用默认配置', self.peaches.options.server.name);
    ...
    getPackers:function () {
        ...
        blockBuckets.forEach(function (bucket) {
            ...
            if (self.peaches.options.sort === 'v') {
    ...
    updatePosition:function (packer, block, x, y) {
        ...
        this.peaches.pom.styleSheet.cssRules.forEach(function (rule, idx) {
            ...
            if (rule.style.getPropertyValue('width') === 'auto') {
                rule.style.setProperty('width', block.canvasImage.width + 'px');
            }
            if (rule.style.getPropertyValue('height') === 'auto') {
                rule.style.setProperty('height', block.canvasImage.height + 'px');
            }
        });
    },
