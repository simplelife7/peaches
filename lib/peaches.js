...
function Peaches(styleText, spriteConfig, next, spriteName) {
    ...
    if (!(this instanceof Peaches)) {
        return new Peaches(styleText, spriteConfig, next, spriteName);
    }
...
Peaches.prototype = {
    _init:function () {
        ...
        this.imageBucket.downloadImageFile(function () {
            self.combo = new Combine(self.imageBucket, self, function (err, server) {
                var styleSheet = self.pom.styleSheet.toString();
                if (server) {
                    styleSheet = styleSheet.replace(new RegExp(server.token, 'g'), server.url);
                }
                self.next(null, styleSheet, self);
            });
        });
    }
};
